  ...
  app.get("/bundle/:id", middleware.requireGlobalToken, function (req, res) {
    ...
    db.bundle
      ...
      .then(function (bundle) {
        responseController.success(res, 200, bundle);
      })
      .catch(function (e) {
                ...
                db.bundle
                  ...
                  .then(function (bundleDeleteStatus) {
                    if (bundleDeleteStatus) {
                      try {
                        const sub_dir =
                          diskDirectory + "/Bundles/" + bundleName;
                        fs.removeSync(sub_dir);
                      } catch (e) {
                        responseController.fail(res, 409, {
                          message:
                            "Bundle deleted successfully but failed to removes relevant files from the database",
                          error: e,
                        });
                      } finally {
      ...
      db.user
        ...
        .then(async function (user) {
          ...
          if (
            ...
            dates.memberships.status == "licensed-instructor" &&
            new Date(dates.end) > new Date() &&
            dates.memberships.courseId == 11
          ) {
            db.bundle
              .findAll({
                where: {
          ...
          if (req.user && req.user.type == "legacy") {
            ...
            where.createdAt = {
              [db.Sequelize.Op.between]: [dates.start, dates.end],
            };
          }
          ...
          db.bundle
            ...
            .then(function (allBundles) {
              ...
              if (user.bundles.length <= 0 && allBundles.length <= 0) {
                ...
                console.log(
                  "USER BUNDLES ARE EMPTY BUT ALL BUNDLES ARE NOT EMPTY"
                );
              } else if (user.bundles.length > 0 && allBundles.length <= 0) {
                returnables = user.bundles;
                console.log(
                  "USER BUNDLES ARE NOT EMPTY BUT ALL BUNDLES ARE EMPTY"
                );
              } else {
                ...
                user.bundles.forEach(function (userBundle) {
                  ...
                  if (index > -1) {
                    returnables.splice(
                      returnables.findIndex(function (i) {
                        return i.id === userBundle.id;
                      }),
                      1
                    );
              ...
              responseController.success(res, 200, returnables);
            });
        })
        .catch(function (e) {
          responseController.fail(res, 406, e);
    ...
    async function (req, res) {
      ...
      if (req.user && req.user.type == "legacy") {
        where.createdAt = {
          [db.Sequelize.Op.and]: [
            {
              [db.Sequelize.Op.notBetween]: [dates.start, dates.end],
              [db.Op.gt]: dates.end,
            },
          ],
        };
    ...
    function (req, res) {
      ...
      if (price === null) {
        responseController.fail(
          res,
          404,
          "Please provide price in request body"
        );
        return;
      }
        ...
        .then(function (bundle) {
          if (bundle) {
            responseController.fail(
              res,
              403,
              "User already bought this bundle"
            );
            return;
          }
          db.user_bundles
